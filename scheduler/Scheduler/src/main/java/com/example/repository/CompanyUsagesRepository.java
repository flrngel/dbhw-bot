...
public interface CompanyUsagesRepository extends PagingAndSortingRepository<CompayUsages, Integer> {
	...
	@Query(value = "select * from company_usages where created_at between :start and :end", nativeQuery = true)
	List<CompayUsages> findByDatesBetween(@Param("start") String start, @Param("end") String end);
